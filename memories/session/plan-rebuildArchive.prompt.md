...
2d. **Implement multi-page merging per (normalized_url, fetch_date)**:
   ...
   - For each date group with multiple pages, merge in deterministic order:
     - Sort by page param value (numeric order, or original filename order if param missing)
     - Concatenate `content_markdown` with clear separator and page marker (e.g., `\n---\n## Page 2\n---\n`)
     - Merge `links` fields (combine + deduplicate per snapshot)
   - Output: Single HistoricalSnapshot per date with merged content
   - *depends on 2c*
